...
if(isfield(info,'ShortDetectors') && (info.ShortDetectors > 0) && (size(probe.detPos,1) == (info.Detectors - info.ShortDetectors)))
    ...
    % seperations were first introduced.
		...
		%find sources and detectors in optodes list
		...
		%get 2D and 3D coords of sources
        DetShort =probe.optodes(lst,:);
        DetShort3D =probe.optodes_registered(lst,:);
		
		%convert sources into dummy short detectors with articial x offsets
        for i=1:length(lst)
		...
		%adjust probes to include:
        %1. non-short detectors
        %2. dummy short detectors
        %3. sources
        %4. landmarks
		...
		%create set 1-to-1 links to dummy SDCs
...
%        height(probe.link),1),[],1),'VariableNames',{'type'})];
	%add support for reading nirsInfo with more than 2 wavelengths
